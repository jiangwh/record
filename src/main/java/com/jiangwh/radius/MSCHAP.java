...
public class MSCHAP {
	...
	public static void parity_key(byte[] szOut, final byte[] szIn, final int offset) {
		int i;
		int cNext = 0;
		int cWorking = 0;
	...
	public static byte[] ChallengeHash(final byte[] PeerChallenge, final byte[] AuthenticatorChallenge,
			final byte[] UserName) throws NoSuchAlgorithmException {
...
//	}
	...
	public static void DesEncrypt(byte[] Clear, int clearOffset, byte[] Key, int keyOffset, byte[] Cypher,
			int cypherOffset) {
	...
	public static byte[] ChallengeResponse(final byte[] Challenge, final byte[] PasswordHash) {
	...
	public static byte[] GenerateNTResponse(byte[] AuthenticatorChallenge, byte[] PeerChallenge, byte[] UserName,
	...
	public static void DesHash(byte[] key, int offsetKey, byte[] Cypher, int offsetCypher) {
		/*
		 * Make Cypher an irreversibly encrypted form of Clear by encrypting
		 * known text using Clear as the secret key. The known text consists of
		 * the string
		 * 
		 * KGS!@#$%
		 */
	 ...
	 * 
	 * @param Password
	 *            The User's Password value in bytes
	 * @param AuthChallenge
	 ...
	 * 
	 * @param UserName
	 *            The User-Name attribute value bytes
	 * @param Password
	 *            The User's Password value in bytes
	 * @param AuthChallenge
	 *            The 16 byte authentication challenge
	...
	public static byte[] doMSCHAPv2(byte[] UserName, byte[] Password, byte[] AuthChallenge)
		...
		byte peerChallenge[] = new byte[16];// RadiusRandom.getBytes(16);
	...
	public static byte[] NTpasswordHashHash(byte[] passwordHash) throws NoSuchAlgorithmException {
		byte[] passwordHashHash = new byte[16];
		MessageDigest messageDigest = MessageDigest.getInstance("MD4");
		messageDigest.update(passwordHash, 0, 16);
		System.arraycopy(messageDigest.digest(), 0, passwordHashHash, 0, 16);
		return passwordHashHash;
	}
}
